...
- [x] **Analítica con consentimiento**: **Google Consent Mode v2** (todo denegado por defecto) + **GA4 diferido** tras consentimiento. **Pendiente cliente**: crear la propiedad GA4 y sustituir `GA_MEASUREMENT_ID = 'G-XXXXXXXXXX'` en `assets/js/velum.js` por el ID real (formato `G-XXXXXXXXXX`). Hasta entonces GA no carga (gate `gaIdIsReal()`).
...
- [x] **Rendimiento/seguridad/a11y (barrido jun. 2026)**: fuentes Google no bloqueantes; preload del póster del hero; vídeo `preload=metadata`; **CSP + HSTS + Permissions-Policy + `upgrade-insecure-requests`** y imágenes `immutable` en `vercel.json`; contraste WCAG AA en formulario/kicker/consentimiento; formulario con `aria-invalid`; pestañas (productos y galería) accesibles por teclado. **Opcional (RGPD)**: autoalojar las fuentes para eliminar el origen Google (hoy van no-bloqueantes pero aún desde Google).
